    ...
    db.vplan.toArray().then(vplan => {
        ...
        for (var i = 0; i < vplan.length; i++) {
            ...
            if (datum1.getTime() != datum2.getTime()) {
                vplanTBody += '<tr><th colspan="6">' + tage[datum2.getDay()] + ', ' + datum2.toLocaleDateString() + '</th></tr>';
                datum1 = datum2;
            }
            ...
            if (vplan[i + 1] != undefined && vplan[i].tag == vplan[i + 1].tag && (vplan[i + 1].stunde - vplan[i].stunde == 1)
                ...
                vplanTBody += '<tr><td>' + vplan[i].stunde + ' + ' + (parseInt(vplan[i].stunde) + 1) +  td + td + vplan[i].bezeichnung + td + vplan[i].raum
                    + td + vplan[i].VLehrer + td + vplan[i].info + '</td></tr>';

                i++;

            } else  vplanTBody += '<tr><td>' + vplan[i].stunde + td  + td + vplan[i].bezeichnung + td + vplan[i].raum
                + td + vplan[i].VLehrer + td + vplan[i].info + '</td></tr>';
